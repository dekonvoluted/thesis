...
Finally, the model is used to predict the feasibility of using the developed CH PLIF system to studying various hydrocarbon combustion systems, with a focus on flame zone imaging.
...
By reducing the reactant flow rate, the range is further extended to 0.82 on the lean side.
...
One possible reason for this divergence is insufficient background-correction on the PLIF images, causing the background signal to be relatively high when compared to LIF signals at their lowest values.
...
To investigate this further, the percentage of methane used to dope a \ce{H2}-air reactant mixture is varied all the way from 0\% (pure \ce{H2}) to 100\% (pure \ce{CH4}).
The predicted signal levels for these mixtures is shown in Figure \ref{fig:methane-addition}.

\input{figures/methane-addition}

The results show that the CH LIF signal increases rapidly with methane addition, reaching a maximum near 20\% \ce{CH4} addition.
It is notable that this maximum signal is higher than that from pure \ce{CH4}.
The rapid increase is attributed to the presence of \ce{H2} in the reacting environment boosting the CH species concentration considerably.
At higher proportions of \ce{CH4}, the signal gradually diminishes due to the more efficient quenching by \ce{CH4} oxidation products like \ce{CO2} and \ce{CO}.
