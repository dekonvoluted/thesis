...
% YOU NEED AN INTRODUCTORY PARAGRAPH SUMMARIZINS WHAT IS IN THIS CHAPTER
...
This thesis, and thus the following background material, focus on the vane-LSB design.
...
The swirler flow and the central flow remain separated for a short distance downstream of the swirler, with this distance called the swirler recess length.
The swirler is located at the head of a constant area nozzle which abruptly expands into the combustion zone. 
%CHECK THE ABOVE DEFN OF RECESS LENGTH
...
The mean axial velocity profile along the centerline of the LSB has been found to exhibit a characteristic linear profile in the near field of the inlet.
% PROVIDE REFERENCE
Two parameters can be used to characterize with this profile.
...
The second parameter is the the axial stretch rate, which is the slope of the linear region of the axial velocity profile.
...
This means that at moderately increasing Reynolds numbers, the divergent flow structure shifts upstream into the injector. This upstream shift ceases for high Reynolds numbers (above 70,000).
...
\input{figures/flameEquilibrium}
...
However, previous studies indicate the LSB flame is robustly stabilized and not prone to blow off.
% ADD REFERENCE
...
%SHOULDN'T YOU REFERENCE DAMKOHLER AND NOT GLASSMAN?
...
%BUT YOU CAN ONLY IGNORE THIS TERM IF u'/Uo IS NOT ALSO SMALL - ??? typically u'/Uo is <0.1 ??? 
...
To do so, the key parameters to be considered are the rms velocity, \(u'\), the laminar flame speed, \(S_L\), the integral length scale of the flow, \(L\), and the flame thickness, \(\delta_f\).
...
Operating in this regime has one major advantage for quantitative LIF measurements, the measured fluorescence signal is nearly independent of the collisional quenching rate, a parameter that can change signficantly throughout the combustion. However, there are drawbacks to operating in this regime.
...
%WHY DO YOU CALL THIS A PSEUDO THREE LEVEL SYSTEM INSTEAD OF CALLING IT A FOUR LEVEL SYSTEM?

\input{figures/simplifiedEnergyLevels}

According to this model, the lower state of the CH system is treated as a single pool from which CH molecules are excited to which they return.
...
%YOU NEED A REFERENCE FOR THE ABOVE SENTENCE
